...
const memoryFiles = new Map<
  string,
  AttachmentFileRecord & { body: ArrayBuffer }
>();
...
const applyAttachmentFileInfoToMemoryItem = (
  ...
  const contentType =
    input.contentType ?? getNonEmptyString(item.data.contentType);
...
const memoryStore: CompatibilityStore = {
  ...
  async associateExistingAttachmentFile(userID, itemKey, input) {
    return associateExistingMemoryAttachmentFile(
      "user",
      userID,
      itemKey,
      input
  ...
  async getAttachmentObjectByStoragePath(storageMd5, storageFilename) {
    for (const file of memoryFiles.values()) {
      if (
        file.storageMd5 !== storageMd5 ||
...
const getMemoryGroupAccess = (userID: number, groupID: number): GroupAccess => {
  const group = getState().groups.get(groupID);
  if (!group) {
    return {
      canAdmin: false,
  ...
  const canEdit =
    canAdmin || (role === "member" && group.data.libraryEditing === "members");
  const canEditFiles =
    group.data.fileEditing !== "none" &&
    (canAdmin || (role === "member" && group.data.fileEditing === "members"));
