...
export const clearMemoryCollections = (
  ...
  if (libraryType && libraryID !== undefined) {
    memoryCollections.delete(
      getMemoryCollectionLibraryKey(libraryType, libraryID)
    );
    return;
  }
...
const memoryCollectionStore: CollectionStore = {
  async createCollections(
    ...
    for (const [index, object] of objects.entries()) {
      ...
      if (
        parentCollection &&
        isMemoryCollectionDescendant(collections, key, parentCollection)
      ) {
  ...
  async getCollection(libraryType, libraryID, collectionKey) {
    const library = getLibrary(getMemoryLibraryID(libraryType, libraryID));
    const collection = getMemoryCollections(libraryType, libraryID).get(
      collectionKey
    );

    return collection
      ? {
          collection: withMemoryCollectionMeta(
            libraryType,
            libraryID,
            collection
          ),
  ...
  async listCollections(libraryType, libraryID, collectionKeys, options = {}) {
    const library = getLibrary(getMemoryLibraryID(libraryType, libraryID));
    const includeMeta = options.includeMeta !== false;
    const collections = [
      ...getMemoryCollections(libraryType, libraryID).values(),
    ]
      .filter(
        (collection) =>
          !collectionKeys || collectionKeys.includes(collection.key)
      )
      .map((collection) =>
        includeMeta
...
const withMemoryCollectionMeta = (
  ...
  const items = [
    ...getLibrary(getMemoryLibraryID(libraryType, libraryID)).items.values(),
  ];
