...
export class PatchAlgorithmUnavailableError extends Error {
  constructor(algorithm: string) {
    super(`Patch algorithm '${algorithm}' is not available in this runtime`);
...
export class PatchApplicationError extends Error {
  constructor(algorithm: string, cause: unknown) {
    super(`Patch algorithm '${algorithm}' failed: ${String(cause)}`);
...
const applyBsdiffPatch = async (
  ...
  try {
    ...
    throw new PatchApplicationError("bsdiff", error);
  } finally {
...
const getXdelta3Instance = async (): Promise<WebAssembly.Instance> => {
  ...
  try {
    ...
    throw new PatchAlgorithmUnavailableError("xdelta");
  }
};
...
const applyXdeltaPatch = async (
  ...
  throw new PatchApplicationError(
    algorithm,
    `decoded output exceeded ${maxXdeltaOutputBytes} bytes`
  );
};
...
const decodeXdeltaMemory = (
  ...
  try {
    ...
    output.set(
      new Uint8Array(xdelta3.memory.buffer, outputPtr, outputSize)
    );

    return {
      output,
      ret,
      str: ret === wasiErrnoSuccess ? "SUCCESS" : readXdeltaCString(xdelta3, xdelta3.xd3_strerror(ret)),
    };
  } finally {
