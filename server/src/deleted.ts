...
const memoryDeletedStore: DeletedStore = {
  async listDeleted(libraryType, libraryID, sinceVersion) {
    ...
    return {
      ...
      version: Math.max(
        0,
        ...getMemoryDeletedEntries(libraryType, libraryID).map((entry) => entry.version)
      ),
    };
  },
};
...
const getMemoryDeletedLibraryKey = (libraryType: LibraryType, libraryID: number) =>
  `${libraryType}:${libraryID}`;

const getMemoryDeletedEntries = (libraryType: LibraryType, libraryID: number) => {
  ...
  const entries: Array<{ key: string; objectType: DeletedObjectType; version: number }> = [];
