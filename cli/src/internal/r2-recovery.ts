...
interface R2CopyOptions extends R2Credentials {
  destinationBucket: string;
  sourceBucket: string;
}

interface R2EmptyOptions extends R2Credentials {
  bucket: string;
}

export const copyR2 = async (options: R2CopyOptions): Promise<void> => {
  if (options.sourceBucket === options.destinationBucket) {
    throw new Error("Source and destination buckets must be different");
  }
  await runR2Recovery(options);
};

export const emptyR2Drill = async ({
  accountID,
  apiToken,
  bucket,
}: R2EmptyOptions): Promise<void> => {
  if (!bucket.includes("-restore-drill-")) {
...
const runR2Recovery = async ({
  accountID,
  apiToken,
  destinationBucket,
  sourceBucket,
}: R2CopyOptions): Promise<void> => {
  ...
  if (
    !(verifyResponse.ok && verifiedToken.success && verifiedToken.result?.id)
  ) {
    throw new Error(
  ...
  const listObjects = async (bucket: string): Promise<R2Object[]> => {
    ...
    do {
      ...
      if (!response.ok) {
        throw new Error(
          `Could not list ${bucket} (${response.status}): ${xml}`
        );
      }
